...
public class DBInitializer implements ApplicationRunner {
    ...
    public DBInitializer(AuthorRepository accountRepository, QuestionRepository questionRepository, AnswerRepository
            answerRepository, VisualizationRepository visualizationRepository, AnnotationRepository annotationRepository, Environment environment) {
    ...
    public void run(ApplicationArguments applicationArguments) throws Exception {
        ...
        if (autofill != null && autofill.equals("true")) {
            ...
            for (int i = 0; i < INITIAL_USERS; i++) {
                ...
                VegaVisualization visualization = new VegaVisualization(account, "Example Vega-Lite Visualization #" + i+1, new LinkedList<>(), vegaSchema);
                this.accountRepository.saveAndFlush(account);
                Question question = new Question("Test", "This is a question");
                this.questionRepository.saveAndFlush(question);

                VegaAnnotation annotation = new VegaAnnotation(account, question, visualization, annotationProperties.get("color").asText(), annotationProperties.get("note"), annotationProperties.get("data"), annotationProperties.get("subject"));
                visualization.addAnnotation(annotation);
